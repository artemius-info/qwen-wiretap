...
const CA_DIR = join(homedir(), '.cq-wiretap');
...
export async function loadOrGenerateCA(): Promise<CAConfig> {
  ...
  const { cert, key } = await generateCACertificate({
    commonName: 'CQ Wiretap CA',
    organizationName: 'CQ Wiretap',
  });
  ...
  console.log(chalk.white(`  sudo cp "${CA_CERT_PATH}" /usr/local/share/ca-certificates/cq-wiretap.crt`));
